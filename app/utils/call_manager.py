...
class CallManager:
    ...
    def initiate_call_request(
        ...
        **kwargs,
        ...
        try:
            ...
            db.add_data(TableConfig.CALL_REQUEST.value, data.id, data.model_dump())
    ...
    def fulfilled_call_request(self, id: str, remarks: Optional[str] = None):
        try:
            ...
            data_ref.update(
                {
                    "status": CallStatus.fulfilled,
                    "remarks": remarks,
                    "fulfilled_time": str(datetime.now()),
                }
            )
    ...
    def cancel_call_request(self, id: str, remarks: Optional[str] = None):
        try:
            data_ref = db.get_doc_ref(TableConfig.CALL_REQUEST.value, id)
            data_ref.update({"status": CallStatus.cancelled, remarks: remarks})
