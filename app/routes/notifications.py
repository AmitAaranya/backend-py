...
async def push_notification(request: PushNotificationRequest):
    ...
    for token in tokens:
        ...
        if token.startswith("ExponentPushToken[") or token.startswith("ExpoPushToken["):
            ...
            logger.warning(f"Invalid push token format for user {request.user_id}: {token}")

    if not messages:
        raise HTTPException(status_code=400, detail="No valid push tokens found for the user.")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(status_code=e.response.status_code, detail=f"Failed to send notification: {e.response.text}")