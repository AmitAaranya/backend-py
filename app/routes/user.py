...
def create_user(payload: CreateUserRequest, role: str = Header("user", alias="X-Role")):
    ...
    user = db.read_data_by_mobile(table_Name, payload.mobile_number)
    if user:
...
def verify_otp(
    mobile_number: str, otp: str, role: str = Header("user", alias="X-Role")
):
    table_name = TableConfig[role.upper()].value
    mobile_number = f"+91{mobile_number.strip()[-10:]}"
    ...
    if is_new_user:
        id = str(uuid.uuid4())
        user = {"id": id, "mobile_number": mobile_number}
...
def fetch_user(mobile_number):
    ...
    if len(mobile_number) < 10:
        raise HTTPException(
            status_code=401, detail=f"Invalid input mobile number {mobile_number}"
        )
    ...
    if not user:
        raise HTTPException(
            status_code=401, detail=f"No User found for number {mobile_number}"
        )
